...
CREATE TABLE `diyi_agreement` (
  ...
  `sign_type` varchar(50) NOT NULL COMMENT '支付状态：待支付，企业已申请支付，企业已支付，平台已支付，已确认收款',
  `sign_date` datetime NOT NULL COMMENT '签署日期',
  `agreement_no` varchar(100) NOT NULL COMMENT '协议编号',
  `sequence_no` varchar(100) NOT NULL COMMENT '顺序号',
  `maker_id` bigint(50) NOT NULL COMMENT '创客ID',
  ...
  `agent_id` bigint(50) NOT NULL COMMENT '渠道商ID',
  `partner_id` bigint(50) NOT NULL COMMENT '合伙人ID',
  `online_aggrement_id` bigint(50) NOT NULL COMMENT '平台在线协议模板ID',
  `online_aggrement_url` varchar(100) NOT NULL COMMENT '在线协议URL',
  `paper_agreement_url` varchar(100) NOT NULL COMMENT '纸质协议URL',
  `third_online_agreement_url` varchar(100) NOT NULL COMMENT '三方在线协议URL',
  `paper_agreement_upload` int(11) NOT NULL COMMENT '纸质协议上传状态',
  `first_side_sign_person` varchar(100) NOT NULL COMMENT '甲方签署人员',
  `second_side_sign_person` varchar(100) NOT NULL COMMENT '乙方签署人员',
  `third_side_sign_person` varchar(100) NOT NULL COMMENT '丙方签署人员',
  `fourth_side_sign_person` varchar(100) NOT NULL COMMENT '丁方签署人员',
...
CREATE TABLE `diyi_online_agreement_need_sign` (
  ...
  `sign_state` int(1) NOT NULL DEFAULT '0' COMMENT '0未签约，1已签约',
  `sign_power` int(1) NOT NULL COMMENT '签字对象性质 甲方；2，乙方；3，丙方；4，丁方',
...
CREATE TABLE `diyi_online_agreement_template` (
  ...
  `template_state` int(11) NOT NULL COMMENT '模板状态 1,应用中；2，已过期。同一个模板上传新模板后，原来的模板即为已过期',
  `sign_state` int(11) NOT NULL COMMENT '签署状态 1，开启中；2，已关闭',
...
CREATE TABLE `diyi_self_help_invoice` (
  ...
  `charge_money_num` decimal(12,2) DEFAULT 0 COMMENT '总价税合计额',
  `service_rate` decimal(12,2) DEFAULT 0 COMMENT '服务税费率',
  `service_and_tax_money` decimal(12,2) DEFAULT 0 COMMENT '总服务税费',
  `service_fee` decimal(12,2) DEFAULT 0 COMMENT '总服务费',
  `service_tax` decimal(12,2) DEFAULT 0 COMMENT '总税',
  `service_invoice_fee` decimal(12,2) DEFAULT 0 COMMENT '总开票手续费',
  `idendity_confirm_fee` decimal(12,2) DEFAULT 0 COMMENT '总身份验证费',
...
CREATE TABLE `diyi_self_help_invoice_detail` (
  ...
  `charge_money_num` decimal(12,2) DEFAULT 0 COMMENT '价税合计额',
  ...
  `service_invoice_fee` decimal(12,2) DEFAULT 0 COMMENT '开票手续费',
  `idendity_confirm_fee` decimal(12,2) DEFAULT 0 COMMENT '身份验证费',
  `create_user` bigint(50) DEFAULT NULL COMMENT '创建人',
  `create_time` datetime NOT NULL COMMENT '创建时间',
  `update_user` bigint(50) DEFAULT NULL COMMENT '更新人',
  `update_time` datetime NOT NULL COMMENT '更新时间',
...
CREATE TABLE `diyi_self_help_invoice_fee` (
  ...
  `total_tax_fee` decimal(12,2) DEFAULT 0 COMMENT '总税费',
  `basic_tax_fee` decimal(12,2) DEFAULT 0 COMMENT '基础税费',
  `basic_tax_fee_rate` decimal(12,2) DEFAULT 0 COMMENT '基础税费率',
  `invoice_fee` decimal(12,2) DEFAULT 0 COMMENT '开票手续费',
  `identify_fee` decimal(12,2) DEFAULT 0 COMMENT '身份验证费',
...
CREATE TABLE `diyi_service_provider` (
  ...
  `invoice_account_name` varchar(50) NOT NULL COMMENT '开票资料-账户名',
  `invoice_account` varchar(50) NOT NULL COMMENT '开票资料-账号',
  `business_service_scope` varchar(50) NOT NULL COMMENT '业务外包产品范围：自然人众包，自然人纵波+分包，个体户众包，个体户总包+分包，个独，有限公司',
  ...
  `contact2_mail` varchar(50) NOT NULL DEFAULT '' COMMENT '联系人2邮箱',
  `spec_demmand` varchar(500) NOT NULL DEFAULT '' COMMENT '特殊需求',
  `service_provider_create_type` varchar(50) NOT NULL COMMENT '创建类型：平台创建，自注册',
  `saler_id` bigint(50) NOT NULL COMMENT '营销人员',
  `runner_id` bigint(50) NOT NULL COMMENT '运营人员',
  `industry_type` varchar(50) NOT NULL COMMENT '行业分类',
  `create_user` bigint(50) DEFAULT NULL COMMENT '创建人',
...
CREATE TABLE `diyi_worksheet` (
  ...
  `worksheet_fee` decimal(12,2) DEFAULT 0 COMMENT '费用',
...
CREATE TABLE `diyi_worksheet_maker` (
  ...
  `check_money` decimal(12,2) DEFAULT 0 COMMENT '验收金额',
