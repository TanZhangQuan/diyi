...
public class IndividualBusinessMakerController {
    ...
    public R queryIndividualBusinessList(@ApiParam(value = "个体户状态") @RequestParam(required = false) Ibstate ibstate, Query query, BladeUser bladeUser) {
        ...
        return individualBusinessService.queryIndividualBusinessListMaker(makerEntity.getId(), null, ibstate, Condition.getPage(query.setDescs("create_time")));
    }
    ...
    public R createIndividualBusiness(@Valid @RequestBody IndividualBusinessEnterpriseAddMakerDTO individualBusinessEnterpriseAddMakerDto, BladeUser bladeUser) {
        //查询当前创客
        R<MakerEntity> result = makerService.currentMaker(bladeUser);
        if (!(result.isSuccess())) {
            return result;
        ...
        return individualBusinessService.createIndividualBusinessMaker(individualBusinessEnterpriseAddMakerDto, makerEntity);
    }
