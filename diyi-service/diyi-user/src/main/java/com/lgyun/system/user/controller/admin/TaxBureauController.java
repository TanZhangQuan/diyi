...
public class TaxBureauController {
    ...
    @ApiOperation(value = "给税局添加匹配服务商", notes = "给税局添加匹配服务商")
    public R addTaxBureauServiceProvider(@ApiParam("服务商ID字符集，每个ID直接用英文逗号隔开") @NotBlank(message = "请选择匹配的服务商！") @RequestParam(required = false) String serviceProviderIds,
                                         @ApiParam("税局ID") @NotNull(message = "税局ID不能为空！！") @RequestParam(required = false) Long bureauId, BladeUser bladeUser){
        //查询当前管理
        R<AdminEntity> result = adminService.currentAdmin(bladeUser);
        if (!(result.isSuccess())) {
            return result;
