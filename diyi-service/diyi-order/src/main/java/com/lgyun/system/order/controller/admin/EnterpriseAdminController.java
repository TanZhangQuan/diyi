...
public class EnterpriseAdminController {
    ...
    public R queryServiceProviderInvoiceCatalogs(@ApiParam(value = "服务商") @NotNull(message = "请选择服务商") @RequestParam(required = false) Long serviceProviderId,
                                                 @ApiParam(value = "开票类目应用范围") @NotNull(message = "请选择开票类目应用范围") @RequestParam(required = false) ApplyScope applyScope, BladeUser bladeUser) {
        //查询当前管理员
        R<AdminEntity> result = userClient.currentAdmin(bladeUser);
        if (!(result.isSuccess())) {
            return result;
    ...
    public R addOrUpdateInvoiceCatalog(@ApiParam(value = "服务商") @NotNull(message = "请选择服务商") @RequestParam(required = false) Long serviceProviderId,
        ...
        return enterpriseProviderInvoiceCatalogsService.addOrUpdateEnterpriseProviderInvoiceCatalog(addOrUpdateEnterpriseProviderInvoiceCatalogDTO, serviceProviderId, enterpriseId);
    }
